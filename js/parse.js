...
var draft = '0'; // 0表示是起草，没有退回来源功能，新需求要示也没有保存功能
...
function parseForm(response, flowAttrParam, draftParam) {
	...
	for(var i = 0; i < resp.Controls.length; i++) {
		...
		switch(item.ControlType - 0) {
			case 0: // 从表，只用于展示数据，不可编辑，不用保存和上传数据
				fragment.appendChild(createSubTable(item));
				break;
			case 1: // 字符文本框
			case 21: // 流水号
			case 2: // 整数文本框
 ...
 * 
 */
function createSubTable(Control) {
	var div = createDiv("item-div");

	div.appendChild(createNameDiv("从表"));

	var valuediv = createDiv("item-value");
	var textdiv = createTextDiv(Control.CaptionName, Control.EditModel);
	textdiv.id = Control.DataField;
	textdiv.style.color = '#6699FF';
...
function createTextarea(Control) {
	...
	if(Control.EditModel == '0') {
		...
		if((Control.ControlType - 0) == 21) {

			textarea.disabled = "disabled";
		}
		if(!isNullStr(Control.Length)) {
...
function createComboBox(Control) {
	...
	if(Control.EditModel == '0') {
		...
		valuediv.className = 'item-value  link';
		valuediv.style.color = '#6699FF';
		valuediv.addEventListener('tap', function() {
...
function createDateType(type, Control) {
	...
	valuediv.innerHTML = (valuediv.value == "" ? "无" : valuediv.value);
...
function createOperBtn() {
	...
	next.addEventListener('tap', function() {
		// signAction在flowManager_detail_ex.html中有定义。
		signAction = false;
...
function event00(Control) {
	mui.openWindow({
		url: 'showSubTable.html',
		id: "showSubTable.html", // 该id在刷新上有使用，不可更改
		show: {
			aniShow: 'pop-in'
		},
		extras: {
...
function event05(textarea, Control) {
	if(Control.ComOpinions.length == 0) {
		mui.toast('没有常用意见');
		return;
	}
...
function event07(valuediv, Control) {
	...
	picker.show(function(selectItems) {
		...
		triggerDateAction(Control.TriggerLinkField, selectItems);
	});
}
...
function triggerDateAction(ctrlName, items) {
	var ctrl = document.getElementById(ctrlName);

	ctrl.innerHTML = items.text;
	ctrl.value = items.value;
}
...
function createDivWithRight(className) {
	var div = createDiv(className);

	var rightImg = document.createElement("img");
	rightImg.src = '/img/right.png';
	rightImg.style = 'width: 16px; position: fixed; right: 10px;';
...
function confirmNext() {
	mui.confirm('您确定要提交下一步吗？', '提示', ['取消', '确认'], function(e) {
		if(e.index == 1) {
			 ...
			 * 注意这里！
			 * 司法局提交后直接流转完成
			 * 神州需要输入签核意见，提交其实就是下一步的操作。做完保存后直接进入签核界面。
			 * 
			 */
			saveOper(true);
		}
...
function getFormValue() {
	...
	try {
		// 将当前节点可编辑的带必填项的意见型带到签核页面，如果有多个就不再传值
		var needItemCount = 0;
		for(var i = 0; i < arr.length; i++) {
			var needItem = arr[i]
			if(needItem.EditModel == '2' && needItem.ControlType == '5') {
				needItemCount++;
				currentOpinion = needItem.Value;
			}
		}
		
		console.log(needItemCount)
